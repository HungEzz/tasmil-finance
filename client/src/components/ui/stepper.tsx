          ...
          <>
            ...
            {variant === "vertical" && (
              <div className="flex gap-4">
                {!isLast && (
                  <div className="flex justify-center ps-[1.25rem]">
                    <StepperSeparator
...
const Description = ({
  ...
  return (
    <Comp
      ...
      className={cn("text-sm text-muted-foreground cursor-pointer", className)}
      {...props}
    >
      {children}
    </Comp>
...
function scrollIntoStepperPanel(
  ...
  if (tracking) {
    // Find the step title element to scroll to
    const stepElement = node?.closest('[data-component="stepper-step"]');
    if (stepElement) {
      // Try to find the step title first
      const titleElement = stepElement.querySelector(
